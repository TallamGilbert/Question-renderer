...
describe("QuestionRenderer — rendering", () => {
  ...
  it("2. renders textarea questions correctly", () => {
    render(
      <QuestionRenderer questions={textareaQuestion} onSubmit={vi.fn()} />,
    );
  ...
  it("3. renders select with all options", () => {
    ...
    expect(
      screen.getByRole("option", { name: /nigeria/i }),
    ).toBeInTheDocument();
  ...
  it("4. renders radio questions with all options", () => {
    ...
    expect(
      screen.getByRole("radio", { name: /frontend/i }),
    ).toBeInTheDocument();
    expect(screen.getByRole("radio", { name: /backend/i })).toBeInTheDocument();
    expect(
      screen.getByRole("radio", { name: /full stack/i }),
    ).toBeInTheDocument();
  });

  it("5. renders checkbox questions with all options", () => {
    render(
      <QuestionRenderer questions={checkboxQuestion} onSubmit={vi.fn()} />,
    );
    ...
    expect(
      screen.getByRole("checkbox", { name: /javascript/i }),
    ).toBeInTheDocument();
  });
});
...
describe("QuestionRenderer — submitted answer types", () => {
  ...
  it("14. submits checkbox answers as an array", async () => {
    ...
    const user = userEvent.setup();
    render(
      <QuestionRenderer questions={checkboxQuestion} onSubmit={onSubmit} />,
    );
